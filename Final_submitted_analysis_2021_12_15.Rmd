...
```{r}
dat=read.csv('data/RAT_phase2_w_PCR.csv') %>%
...
#  filter(vvf=='yes') %>%
  ...
  mutate(familyid = paste(mobile,sample_id,sep='_')) %>%
...
#table(duplicated(dat$mobile))

#aa=dat %>% filter(mobile %in% dat$mobile[duplicated(dat$mobile)==TRUE])
...
#%>%
  select(mobile, vvf, age,vvf_saliva,vvf_nasal,PCR_result)
...
figdat$testessay = factor(figdat$testessay , levels=c(' saliva',' nasal',' both'))
 ...
 png('Fig1_sssp.png',width=3000, height=1200, res=300)
